        ...
        <div className="border-t pt-4 mt-4">
          <div className="flex items-center justify-between text-sm text-gray-600 dark:text-gray-400">
            <div className="flex items-center space-x-4">
              {/* Stars */}
              <div className="flex items-center space-x-1" title={`${formatNumber(repository.stargazers_count)} stars`}>
                <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  <path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z" />
                </svg>
                <span aria-label={`${formatNumber(repository.stargazers_count)} stars`}>
                  {formatNumber(repository.stargazers_count)}
                </span>
              </div>

              {/* Forks */}
              <div className="flex items-center space-x-1" title={`${formatNumber(repository.forks)} forks`}>
                <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
              ...
              {repository.open_issues_count > 0 && (
                <div className="flex items-center space-x-1" title={`${formatNumber(repository.open_issues_count)} open issues`}>
                  <svg className="h-4 w-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                    <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm-2 15l-5-5 1.41-1.41L10 14.17l7.59-7.59L19 8l-9 9z" />
                  </svg>
                  <span aria-label={`${formatNumber(repository.open_issues_count)} open issues`}>
            ...
            {/* Last Updated */}
            <div className="text-xs" title={`Last updated: ${formatDate(repository.updated_at)}`}>
